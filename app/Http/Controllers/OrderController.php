...
class OrderController extends Controller
{
    ...
    /**
     * Endpoint polling lokasi kurir untuk halaman tracking customer.
     * Resolusi order aktif IDENTIK dengan tracking() — order paling baru
     * berstatus dijemput/dikirim/siap, fallback ke order terakhir. Hanya
     * customer pemilik order yang boleh akses.
     */
    public function customerTrackingLocation()
    {
        $order = Auth::user()->customerOrders()
            ->with('driver')
            ->whereIn('status', ['dijemput', 'dikirim', 'siap'])
        ...
        // abort_if di atas sudah menjamin order milik customer ini
        // (di-query lewat customerOrders()), tapi kita tetap eksplisit
        // untuk kejelasan kontrak ownership.
        abort_if($order->customer_id !== Auth::id(), 403);

        $hasDriver = $order->driver_lat !== null && $order->driver_lng !== null;

        return response()->json([
            'has_driver' => $hasDriver,
            'lat' => $hasDriver ? (float) $order->driver_lat : null,
            'lng' => $hasDriver ? (float) $order->driver_lng : null,
    ...
    /**
     * Terima koordinat GPS kurir untuk pesanan yang sedang aktif
     * dikerjakannya. Dipanggil via polling/watchPosition dari device
     * kurir. Hanya kurir pemilik tugas yang boleh kirim, dan hanya
     * saat order berstatus aktif (dijemput/dikirim).
     */
    public function updateDriverLocation(Request $request, Order $order)
    {
        abort_if($order->driver_id !== Auth::id(), 403);

        // Hanya terima lokasi saat ada tugas jalan (jemput / antar).
        // Status lain (workshop, selesai, dibatalkan) tidak relevan.
        if (! in_array($order->status, ['dijemput', 'dikirim'], true)) {
            return response()->json([
                'ok' => false,
                'message' => 'Order tidak dalam status aktif.',
            ], 422);
        }
