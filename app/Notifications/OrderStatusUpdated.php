...
class OrderStatusUpdated extends Notification implements ShouldQueue
{
    ...
    public function via(object $notifiable): array
    {
        ...
        // Email hanya untuk customer yang memang punya alamat email
        // (email opsional saat daftar — HP wajib). Admin/driver cukup
        // notifikasi in-app agar inbox-nya tidak penuh per order.
        if (($notifiable->role ?? null) === 'customer' && ! empty($notifiable->email)) {
            $channels[] = 'mail';
        }
    ...
    /**
     * Email versi (case-specific) untuk customer — layout branded Azka.
     */
    public function toMail(object $notifiable): MailMessage
    {
        $detailUrl = null;
        try {
            $detailUrl = route('customer.order.detail', ['order' => $this->order->id]);
        } catch (\Throwable $e) {
            // route tidak tersedia di konteks tertentu — biarkan tanpa CTA.
        }

        return (new MailMessage)
            ->subject($this->title.' — Pesanan #'.$this->order->order_code)
            ->view('emails.order-status', [
                'customerName' => $notifiable->name ?? 'Pelanggan',
                'title' => $this->title,
                'bodyText' => $this->message,
                'orderCode' => $this->order->order_code,
                'statusLabel' => $this->order->status_label,
                'statusColor' => $this->order->status_color,
                'url' => $detailUrl,
            ]);
    }
