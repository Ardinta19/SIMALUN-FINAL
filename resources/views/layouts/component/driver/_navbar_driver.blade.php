...
@php
    ...
    $driverTaskCount = 0;
    try {
        $driverTaskCount = auth()->user()?->driverOrders()->whereIn('status', ['dijemput', 'dikirim'])->count() ?? 0;
    } catch (\Exception $e) {}
@endphp

<nav class="driver-nav" id="driver-nav" aria-label="Navigasi utama">
    <div class="driver-nav__inner">
        ...
        <a href="{{ route('driver.orders') }}"
           ...
           @if($navActive === 'tugas') aria-current="page" @endif
           aria-label="Tugas">
            <span class="driver-nav__icon">
...
.driver-nav__item.is-active { color: #0077b6; }
...
@media (prefers-reduced-motion: reduce) { .driver-nav__item { transition: none; } }
</style>
