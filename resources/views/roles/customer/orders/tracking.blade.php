...
<div id="map-container">
    ...
    <div class="map-status-chip">
        ...
        <span class="map-status-label" id="js-status-label">Menunggu kurir membagikan lokasi…</span>
    </div>
    ...
    <div class="panel-inner">
        ...
        <div class="eta-card">
            ...
            <div class="eta-info">
                ...
                <div class="eta-sub" id="js-eta-sub">Menunggu kurir membagikan lokasi…</div>
            </div>
...
@php
    // Titik rumah customer = koordinat alamat pesanan (jika sudah disimpan saat
    // buat alamat/pesanan). Fallback ke titik tengah area layanan kalau belum ada.
    $addr = $order->customerAddress;
    $homeLat = $addr && $addr->latitude !== null ? (float) $addr->latitude : null;
    $homeLng = $addr && $addr->longitude !== null ? (float) $addr->longitude : null;
    $fallbackLat = -1.603137;
    $fallbackLng = 103.606789;
@endphp

<script>
    document.addEventListener('DOMContentLoaded', function () {
        const LOCATION_URL = @json(route('customer.tracking.location'));
        const POLL_MS = 15000;

        // Titik rumah customer: pakai koordinat alamat tersimpan bila ada,
        // selain itu fallback ke titik tengah area (lalu boleh dikoreksi GPS device).
        const HOME_LAT = @json($homeLat);
        const HOME_LNG = @json($homeLng);
        let customerLat = HOME_LAT !== null ? HOME_LAT : {{ $fallbackLat }};
        let customerLng = HOME_LNG !== null ? HOME_LNG : {{ $fallbackLng }};
        let hasCustomerLocation = HOME_LAT !== null;

        const statusLabelEl = document.getElementById('js-status-label');
        const etaSubEl = document.getElementById('js-eta-sub');
        ...
        // Driver marker & route dibuat saat lokasi kurir REAL tersedia.
        ...
        if (HOME_LAT === null && 'geolocation' in navigator) {
            navigator.geolocation.getCurrentPosition(function (pos) {
                ...
                if (routeLine) {
                    routeLine.setLatLngs([[driverMarker.getLatLng().lat, driverMarker.getLatLng().lng], [customerLat, customerLng]]);
                }
                fitMap();
            }, function () {
                // Izin ditolak / tidak tersedia — pakai fallback (sudah di-set).
            }, { enableHighAccuracy: true, maximumAge: 30000, timeout: 12000 });
        }

        // ── Relative time (mirip diffForHumans) ───────────────────
        function diffForHumans(iso) {
            if (!iso) return '';
        ...
        function showWaiting() {
            ...
            if (statusLabelEl) statusLabelEl.textContent = 'Menunggu kurir membagikan lokasi…';
            if (etaSubEl) etaSubEl.textContent = 'Menunggu kurir membagikan lokasi…';
        }

        function updateDriver(lat, lng, updatedAtIso, driverName) {
            const latlng = [lat, lng];
            if (!driverMarker) {
                driverMarker = L.marker(latlng, { icon: driverIcon })
                    .addTo(map)
                    .bindPopup(driverName ? ('Kurir ' + driverName) : 'Kurir Azka Laundry');
                routeLine = L.polyline([latlng, [customerLat, customerLng]], {
                    color: '#0077b6',
                    weight: 3,
                    opacity: 0.6,
                    dashArray: '8, 6',
                ...
                routeLine.setLatLngs([latlng, [customerLat, customerLng]]);
            }
            if (statusLabelEl) statusLabelEl.textContent = 'Kurir Menuju Lokasi';
            if (etaSubEl) {
                const rel = diffForHumans(updatedAtIso);
                etaSubEl.textContent = rel ? ('Lokasi kurir diperbarui ' + rel) : 'Kurir sedang dalam perjalanan';
            }
        }
        ...
        function poll() {
            fetch(LOCATION_URL, {
                ...
                if (data && data.has_driver && data.lat !== null && data.lng !== null) {
                    updateDriver(data.lat, data.lng, data.updated_at, data.driver_name);
                } else {
                    showWaiting();
                }
            }).catch(function () {
                // Jaringan error — biarkan state terakhir, coba lagi di poll berikutnya.
            });
        }
